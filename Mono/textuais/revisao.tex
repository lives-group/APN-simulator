...
Uma das linguagem que se é possivel reconhecer com um APD é a linguagem $\{a^n b^n \mid n \geq 0\}$, que pode ser definida formalmente pela seguinte sêxtupla: $({1,2},{a,b},{X},\delta,1,{1,2})$, em que $\delta$ é dada por: 
...
Uma das linguagem que se é possivel reconhecer com um APN é a linguagem $\{w \in\{0,1\}^* \mid n0(w) = n1(w)\}$, que pode ser definida formalmente pela seguinte sêxtupla: $({=,\neq},{0,1},{Z,U},\delta,1,{=,=})$, em que $\delta$ é dada por: 
...
para a palavra 0101 a execução do APD é dada por:
...
O conceitos de derivadas pode ser extendido para linguagens livres de contexto adicionado-se os caso necessários para a derivação de símbolos não terminais
que estão presentes nessas gramáticas. Uma GLC é formada por um $\langle \Sigma,V,R,P \rangle$,

Essa mudança afeta tanto a definição de anulável quanto a definição de derivadas. Intuitivamente, a nova definição de
anulável para GLC deve receber um forma sentencial e retornar \texttt{T} se essa forma poder derivar $\epsilon$. Vamos usar
letras gregas do início do alfabeto ($\alpha$ e $\beta$)para denotar formas sentenciais da gramática e a letra $v$ para denotar um
não terminal. Logo a nova definição de anuláveis
seria $null_g :\{\Sigma \cup V\}^* \to \{T,F\}  $
...
\begin{array}{lcl}
    ...
    null_g(\alpha \;|\; \beta) & = & null(\alpha) \vee null(\beta)\\
    null_g(v) & = & null(\alpha), \texttt{ se } v \to \alpha \in R\\
\end{array}
\]

Subsequentimente a definição de derivada se torna:
...
d_g(a,\alpha \,\beta) & = & \left\{\begin{array}{l}
                               d_g(a,\alpha)\,b, \texttt{ se } \neg null_g(\alpha)\\
                               d_g(a,\alpha)\,b \;|\; d_g(a,b), \texttt{ se } null_g(\alpha)\\
                          \end{array}\right.\\
...
Essas definições são matematicamente corretas, mas como observado por Migth \cite{Might2011}, computacionalmente elas podem levar
a recursão inifinita caso a gramática possua recursão à esquerda. A implementação proposta por Migth tem como objetivo o uso de derivadas para
parsing e emprega o uso de linguagem funcional com suporte a avalição lazy e memoização para contornar esse problemas. Além desses problemas Migth também precisou implementar simplifcações após cada passo da derivada para evitar crescimento rápido da gramática e a degradação do tempo de parsing.

Uma segunda abordagem para derivadas proposta por \cite{Henriksen2019} descreve um método com custo quadrático em espaço e cúbico em tempo
para realização de parsing. A abordagem consiste construir uma nova GLC a partir da gramática original. Além disso cada não terminal é
anotado com o sequencia de símbolos sonre os quais foi derivado até um dado momento. Essa abordagem permite que se aplique derivadas
a gramáticas com recursão à esquerda. Uma segunda otimização é a eliminação de regras desnecessários, que são inatingíveis a partir do símbolo de
partida.
