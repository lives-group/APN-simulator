...
Em seguida, o \ac{GAM} \cite{GAM}, que é um sistema desenvolvido por Anibal Jukemura, Hugo Nascimento e Joaquim Uchôa na UFLA no ano de 2005. Tem a capacidade de representar em forma de grafos direcionados \ac{AFD}, \ac{AFN}, \ac{AFNL}, \ac{APD}, \ac{APN} e \ac{MT}, além de reconhecer se uma palavra pertence ou não à máquina de estados representada por esse grafo. O GAM também consegue transformar AFNs em AFDs e minimizar AFDs.
...
\ac{jFAST} \cite{jFAST} é um software que foi desenvolvido na Universidade de Villanova por Timothy M. White e Thomas P. Way no ano de 2006 utilizando a linguagem de programação Java e traz a proposta de construir diagramas dinâmicos para simular o comportamento de AFDs, AFNs, APDs e máquinas de Turing.

\ac{FSMD} é um aplicativo web desenvolvido por Evan Wallace em 2010 com o objetivo de criar com facilidade diagramas que representassem máquinas de estados em geral e portar esses diagramas para vários formatos como PNG, JSON e LaTeX, porém não simula seu funcionamento, apenas desenha as máquinas.
...
Dado um alfabeto $\Sigma$, uma \ac{ER} sobre $\Sigma$ é definida indutivamente pelo seguinte conjunto de regras:

\begin{itemize}
 \item $\emptyset$ denota a linguagem vazia.
 \item A palavra vazia $\epsilon$ é uma \ac{ER}.
 \item Se $a \in \Sigma$, então $a$ é uma \ac{ER}.
...
Um autômato de pilha pode aceitar palavras de três formas: por estado final, por pilha vazia ou por ambos. No primeiro caso, a palavra é aceita se, após a leitura de todos os símbolos da palavra, o autômato estiver em um estado final. No segundo caso, a palavra é aceita se, após a leitura de todos os símbolos da palavra, a pilha do autômato estiver vazia. No terceiro caso, a palavra é aceita se atender às duas condições.
...
\begin{itemize}
 \item \textbf{Aceitação por estado final} : Conjunto de todas as palavras tais que o APD/APN consome toda a palavras e termina em estado final.
 \item \textbf{Aceitação por pilha vazia} : Conjunto de todas as palavras tais que o APD/APN consome toda a palavras e termina com pilha vazia.
 \item \textbf{Aceitação por estado final e pilha vazia}: Conjunto de todas as palavras tais que o APD consome toda a palavras e
       termina em estado final e com pilha vazia.
\end{itemize}

No caso particular de um APN considera-se que e qualquer caminho de computação satisfaz a condição de aceitação escolhida, então a palavra é aceita.
...
\begin{align}
>>>>>>> 9f1527f56d8073b5da1964dafde2b17098842ed2
    & null_g(\epsilon)  =  T\\
    & null_g(a)  =  F\\
    & null_g(\alpha \, \beta)  =  null_g(\alpha) \wedge null_g(\beta)\\
    & null_g(\alpha \;|\; \beta)  =  null_g(\alpha) \vee null_g(\beta)\\
