...
A classe \texttt{APNRunner} é a principal responsável por emular o comportamento dinâmico do autômato sobre uma dada entrada. Ela possui o atributo \texttt{apn}, que é um objeto da classe \texttt{APN}; \texttt{input}, que é a palavra que será executada; \texttt{snaps}, que é um \emph{array} de objetos da classe \texttt{Snapshot}; e \texttt{graph}, que é o grafo rastreador da execução. Além destes, a classe armazena informações de controle, como o tipo de critério de aceitação esperado (\texttt{accType}) e o limite de \emph{loops} estipulado (\texttt{limit}).
